...
- 2026-06-12 17:10 PDT: started the LCD-safe APS sidecar demo milestone on branch `lcd-safe-aps-sidecar-demo`, adding compile-flag-gated sidecar hooks inside the cooperative dashboard flow and leaving the default boot path unchanged.
- 2026-06-12 17:25 PDT: rebuilt the sidecar demo with `APP_INSULIN_PUMP_MODE=1`, flashed the corrected image, and confirmed via UART that the LVGL boot banner and a single `APS sidecar:` line appear after reset-run; the sidecar remains a 5000 ms cooperative service rather than a boot-time probe or smoke loop.
